 $( document ).ready(function() {
	...
	/*
	$(".depth2_dropdown").each(function(){
		if($(this).hasClass("active")){
			$(".dropdown_title").text($(this).find("a:first").text());
		}
	});
	*/
	...
	setTimeout(function(){ $(".loding-box").hide(); }, 1000);
...
function fn_headerLogin(id, pwd, url){
	$item				= {};
	$item.SYSTEM_GB		= 'ANGE';
	$item.id			= $(id).val();
	$item.password		= $(pwd).val();
...
function fn_menuUrlLink(menuId, link, subCnt){
	...
	if(!auth){
		...
		if(fnIsNull($rootScope.uid)){
			dialogs.notify('알림', '로그인 후 사용 가능 합니다.', {size: 'md'});
			//fn_login();
		}else{
			location.href = link;
		}
...
function fn_prevMenu(no){
	var obj			= $("li[menuNo='"+no+"']");
	var menuDepth	= $(obj).attr('menuDepth');
	if(menuDepth == 1){ // 2차메뉴라면
		var $target	= $(obj);
		var aIndex = $target.index();
		if(aIndex == 0){
			var aTarget	= $target.parent().children().eq($target.siblings().length);
		} else {
			var aTarget	= $target.parent().children().eq(aIndex - 1);
		}
	} else { // 3차메뉴라면
		var $target		= $(obj).parent().parent(); // 내 상위메뉴
		var aIndex		= $target.index(); // 내 상위메뉴의 순번
		if(aIndex == 0){ // 내 상위메뉴가 첫번째
			var aTarget	= $target.parent().children().eq($target.siblings().length);
		} else {
			var aTarget	= $target.parent().children().eq(aIndex - 1);
		}
	...
	var aNo = aTarget.attr("menuNo");

	//console.log("menuDepth : " + menuDepth + "\r\naIndex : "+aIndex+"\r\nlength : "+$target.siblings().length+"\r\naNo : "+aNo);

	$(".second_depth").each(function(){
		if($(this).attr("menuNo") == aNo){
			$(this).trigger("click").next("ul").find("a:first").trigger("click");
		}
	});
}

function fn_nextMenu(no){
	var obj			= $("li[menuNo='"+no+"']");
	var menuDepth	= $(obj).attr('menuDepth');
	if(menuDepth == 1){ // 2차메뉴
		var $target	= $(obj);
		var aIndex = $target.index();
		if(aIndex == $target.siblings().length){
			var aTarget	= $target.parent().children().eq(0);
		} else {
			var aTarget	= $target.parent().children().eq(aIndex + 1);
		}
	} else {
		var $target		= $(obj).parent().parent(); // 내 상위메뉴
		var aIndex		= $target.index(); // 내 상위메뉴의 순번
		if(aIndex == $target.siblings().length){ // 내 상위메뉴가 첫번째
			var aTarget	= $target.parent().children().eq(0);
		} else {
			var aTarget	= $target.parent().children().eq(aIndex + 1);
		}
...
$(document).on("click", ".moveMenu", function(){
	...
	var nowActivate = 0;
	var nextActivate = 0;
	var dropdown_size = $(".depth2_dropdown").length;

	$(".depth2_dropdown").each(function(){
		if($(this).hasClass("active")){
			nowActivate = $(this).index();
		}
	});
	...
	switch(moveType){
		case "prev":
			if(nowActivate == 0){
				nowActivate = dropdown_size - 1;
			} else {
				nowActivate--;
			}
			break;
		case "next":
			if(nowActivate == dropdown_size){
				nowActivate = 0;
			} else {
				nowActivate++;
			}
	...
	var $target = $(".depth2_dropdown").eq(nowActivate);
	var link1 = $target.attr("data-link1");
	var link2 = $target.attr("data-link2");
	var link3 = $target.attr("data-link3");

	console.log("nowActivate : " + nowActivate + "\r\nlink1 : " + link1 + "\r\nlink2 : " + link2);

	//$(".depth2_dropdown").eq(nowActivate).find("a").trigger("click");
	fn_menuUrlLink(link1,link2,link3);
});
