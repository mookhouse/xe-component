<?php
/**
 * @class ange_bbs_recommend
 * @author singleview.co.kr
 * @brief 게시판 페이지 추천글 추출
 * @version 0.0.1
 **/

class ange_bbs_recommend extends WidgetHandler 
{
 ...
 */	
	public function proc($oArgs) 
	{
		...
		$aDocList = [];
		$nCurDocSrl = Context::get('document_srl');
		if(count($oRst->data)) 
		{
			foreach($oRst->data as $nIdx => $oSingleDoc) 
			{
				if($nCurDocSrl == $oSingleDoc->document_srl) // 현재 문서는 추천 제외
					continue;
